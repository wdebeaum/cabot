 ...
 * Time-stamp: <Thu Sep 20 15:05:00 EDT 2001 ferguson>
 *
...
static void doAudioWrite(char *buf, int len);
...
ttCB(char *buf, int count, int rec_size, struct TtsIndex *index)
{
	...
	if (nout >= AUDIOBUFSIZE-1) {
	    ...
	    doAudioWrite((char*)audiobuf, nout*sizeof(short));
