...
def read_pkl_frame_from_socket(host,port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect((host,port))
    #data_sock, addr = list_sock.accept()
    result = ""
    while True:
        data = sock.recv(1024)
        if not data:
    ...
    #socket_file = data_sock.makefile('rb', SOCKET_BUFFER_SIZE)
    #print 'Length ' + str(socket_file.tell())
    print len(result)
    ...
    with open('color_output','w') as output_file:
        try:
            ...
            while True:
                ...
                frame = read_pkl_frame_from_socket(hostname,port)
                print 'read frame'
                if frame == None:
                    indent(xml_root)
