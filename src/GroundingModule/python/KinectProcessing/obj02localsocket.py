"""Ian Perera
    ...
    Use this code if TRIPS and the feature extraction code can run on the same machine.
    """
...
def new_xml_root(utterance_num):
    global Z_FEATURES
    xml_root = xml.Element('session')
    xml_root.attrib['name'] = 'Speech' + str(utterance_num)
...
def read_start_end_times_from_socket(host,port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.connect((host,port))
    
    time_data = sock.recv(1024)
    time_data_split = time_data.split('\n')
    start_time = time_data_split[0]
    end_time = time_data_split[1]
    sock.close()
...
def send_feature_data_to_socket(host, port, filename):
...
def main():
    ...
    utterance_num = 0
    xml_root = new_xml_root(utterance_num)
    ...
    with open('color_output','w') as output_file:
        try:
            ...
            #list_sock = initialize_socket(port)
            ...
            while True:
                ...
                (start_time, end_time) = \
                        read_start_end_times_from_socket("localhost",port)
                print "start: " + start_time + " end: " + end_time
                frame_list = get_files_for_time(directory, start_time, end_time)
                for frame_name in frame_list:
                    frame = laceutils.read_pkl_frame(directory + frame_name)
                    
                    print 'reading frame at' + directory + frame_name
                    if frame == None:
                        indent(xml_root)
                        tree = xml.ElementTree(xml_root)
                        temp_filename = 'features-' + str(utterance_num) + '.xml'
                        tree.write(temp_filename)
                        send_feature_data_to_socket(hostname, xml_port, temp_filename)
                        utterance_num += 1
                        xml_root = new_xml_root(utterance_num)
                        continue
            ...
            indent(xml_root)
            tree = xml.ElementTree(xml_root)
            tree.write('color_output-socket.xml')
            pass
        except IndexError:
            print 'IndexError'
            indent(xml_root)
