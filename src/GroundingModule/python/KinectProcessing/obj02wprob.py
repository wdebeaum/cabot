...
# gz2 = higher = further back back plane
...
BATCH_MODE = False # Disables visualization and optical flow processing
BATCH_BLOB_MODE = False # Disables blob visualization
time_color_dict = {}
...
rgb_prob_element = xml.SubElement(spaces_element,'rgb_prob')
xml.SubElement(rgb_prob_element, 'dim').text = '3'
xml.SubElement(rgb_prob_element, 'type').text = 'euclidean'
...
def get_blob_element(mask, rect, skel, num_blob_pixels, color, color_variance, label):
...
##        numpy.putmask(half_matrix_depth,depthmask, 5)
        ...
        rgb_weighted_average = numpy.sum(numpy.sum(prob_vector * rgb ,axis=0),axis=0) \
                               / numpy.sum(prob)
##        rgb_weighted_average = numpy.sum(numpy.sum(prob_vector * hsv ,axis=0),axis=0) \
##                               / numpy.sum(prob)
...
##        rgb_weighted_average_element.attrib['r'] = str(rgb_weighted_average[2])
        ...
        if output_file is not None and blobidxset is not None:
            blue_channel = rgb[:,:,0]
            green_channel = rgb[:,:,1]
            red_channel = rgb[:,:,2]
            ...
            for blob_num in range(0, max_idx):
                ...
                if num_blob_pixels <= 0:
                    print "Skipping blob num: ", blob_num
                    continue
                ...
                print 'blob_mask shape: ', blob_mask.shape
...
#                blob_color_rect_luv = cv2.pyrMeanShiftFiltering(blob_color_rect_luv,50,60,
#                                                                maxLevel = 0)
...
#                bgr_result= cv2.cvtColor(blob_color_rect_luv, cv2.COLOR_LUV2BGR)
                ...
                hu_element = get_hu_moments_element(blob_mask)
                features_element.append(hu_element)
                ...
                subblobs_element = xml.Element('subblobs')
                
                #Edited out for demo
#                for component in contours:
#                    if (component[0] > 10 and (component[2][2] < rects[blob_num][2] or
#                        component[2][3] < rects[blob_num][3])):
#                        print component
#                        cv2.rectangle(bgr_result, (component[2][0], component[2][1]),
#                                    (component[2][0] + component[2][2], component[2][1] + component[2][3]),
#                                     (0, 0, 255), 1)
#                        subblob_element = xml.SubElement(subblobs_element, 'subblob')
#                        subblob_element.attrib['x'] = str(component[2][0])
#                        subblob_element.attrib['y'] = str(component[2][1])
#                        subblob_element.attrib['width'] = str(component[2][2])
#                        subblob_element.attrib['height'] = str(component[2][3])

                        
                #cv2.imshow('blob' + str(blob_num), bgr_result)
                #cv2.waitKey(10)
                #cv2.imshow('blobmask' + str(blob_num), blob_mask_rect)
                #cv2.waitKey(10)
...
#        cv2.ellipse(cmask, track_box, (0, 0, 255), 2)
    ...
    if not BATCH_MODE:
        ...
        if g_rgbfeat != None:
            ...
            f = cv2.calcOpticalFlowFarneback(g_rgb, grayrgborig, .5, 3, 15, 3, 5, 1.2, 0)
            print f.shape
